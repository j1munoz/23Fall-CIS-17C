...
void Game::finalScores() {
    ...
    auto minIter = min_element(player.begin(), player.end());
    auto maxIter = max_element(player.begin(), player.end());

    Player min = *minIter;
    Player max = *maxIter;
    auto minIndex = distance(player.begin(), maxIter);
    auto maxIndex = distance(player.begin(), maxIter);
   
    while(max.getScore() > 21) {
        player[maxIndex].assignScore(0);
        maxIter = max_element(player.begin(), player.end());
        max = *maxIter;
        maxIndex = distance(player.begin(), maxIter);
    }
