...
void Game::playAI() {
    currentState = "AI_Start";

    // Begin the game
    dealer = new Dealer();

    while (currentState != "AI_End") {
        handleState(currentState);
    }

    currentState = "Dealer_Start";

    // Dealer's turn
    while (currentState != "Dealer_End") {
        handleState(currentState);
    }
}
...
void Game::resetGambler() {
    // Sort the gamblers
    deque<Gambler> temp(gamblers->begin(), gamblers->end());
    mergeSort(temp, 0, temp.size() - 1);
    gamblers->assign(temp.begin(), temp.end());
...
void Game::finalScores() {
    ...
    for (int i = 0; i < numPlayers; i++) {
        ...
        if(current.getScore() <= 21 && current.getScore() > winner.getScore())
            winner = current;
        cout << "\nValues that were added to the score: ";
        current.showValues();
        cout << "\n\n";
        players.pop();
        players.push(current);
    ...
    // Output the remaining cards left
    dealer->displayRemain();
}
...
void Game::initializeStates() {
    addState("AI_Start", "AI_Draw");
    addState("AI_Draw", "AI_End");
    addState("Dealer_Start", "Dealer_Draw");
    addState("Dealer_Draw", "Dealer_End");
}

void Game::addState(string from, string to) {
    gameState[from].push_back(to);
}

bool Game::isValidState(string from, string to) {
    auto states = gameState.find(from);
    if(states != gameState.end()) 
        return find(states->second.begin(), states->second.end(), to) != states->second.end();
    return false;
}

void Game::handleState(string state) {
    if (state == "AI_Start") {
        cout << "CURRENT STATE: AI_Start\n\n";
        cout << "It is now " << ai->getName() << "'s turn, the AI.\n";
        cout << "Drawing your first card...." << endl;
        ai->addToHand(dealer->deal());
        cout << "Drawing your second card...." << endl;
        ai->addToHand(dealer->deal());
        currentState = "AI_Draw";
    } else if (state == "AI_Draw") {
        cout << "CURRENT STATE: AI_Draw\n\n";
        do {
            if(ai->getScore() <= 19) {
        ...
        currentState = "AI_End";
    } else if (state == "Dealer_Start") {
        cout << "CURRENT STATE: Dealer_Start\n\n";
        cout << "\n\nIt is now " << dealer->getName() << "'s turn, our dealer.\n";
        cout << "Drawing your first card...." << endl;
        dealer->addToHand(dealer->deal());
        cout << "Drawing your second card...." << endl << endl;
        dealer->addToHand(dealer->deal());
        currentState = "Dealer_Draw";
    } else if (state == "Dealer_Draw") {
        cout << "CURRENT STATE: Dealer_Draw\n\n";
        do {
            if(dealer->getScore() <= 19) {
                cout << dealer->getName() << " decides to draw again." << endl;
                cout << "Here is your next card...." << endl;
        ...
        currentState = "Dealer_End";
    }
}

void Game::merge(deque<Gambler>& arr, int left, int middle, int right) {
    int n1 = middle - left + 1;
    int n2 = right - middle;

    deque<Gambler> leftArray(arr.begin() + left, arr.begin() + left + n1);
    deque<Gambler> rightArray(arr.begin() + middle + 1, arr.begin() + middle + 1 + n2);
...
void Game::mergeSort(deque<Gambler>& arr, int left, int right) {
    if (left < right) {
