...
void Player::addToHand(Card card) {
    ...
    if(hand.find(card) == hand.end()) {
        ...
        if(aceCheck[0] == 'A') {
            score += assignAce(card);
            valueTree.tree = valueTree.insertNode(valueTree.tree, assignAce(card));
        }
        else {
            score += card.getValue();
            valueTree.tree = valueTree.insertNode(valueTree.tree, card.getValue());
        }
        hand.insert(card);
        showScore();
    }
...
void Player::showValues() {
    valueTree.traverse(valueTree.tree);
}

void Player::deleteValues() {
    valueTree.deleteTree(valueTree.tree);
}
